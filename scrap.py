...
#     # Find the price table (it's the first <table> on the page)
...
#             if price != "":
#                 price_data[f"{month} {year}"] = float(price)

#     return price_data

# def get_latest_price():
#     price_data = fetch_uranium_price_table()
#     # Get the most recent entry (last key when sorted by year/month)
#     latest_entry = list(price_data.items())[-1]
#     return latest_entry

# if __name__ == "__main__":
#     uranium_data = fetch_uranium_price_table()
#     for k, v in uranium_data.items():
#         print(f"{k}: {v}")

#     latest_date, latest_price = get_latest_price()
#     print(f"\nLatest Uranium Price ({latest_date}): ${latest_price}/lb")
